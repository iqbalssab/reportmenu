    ...
    public function membertidur()
    {
            $dbProd = db_connect('production');
                ...
                $membertidur = $dbProd->query(
                    "SELECT kodemember, namamember, tgl_akhir, alamat2, alamat4, sisa_saldo, hpmember, ktp 
                    FROM (
                    select
                    ...
                    right join (select jh_cus_kodemember as kodemember, max(trunc(jh_transactiondate)) as tgl_last_belanja 
                          from tbtr_jualheader
                          where trunc(jh_transactiondate) <= trunc(sysdate) - 90
                    ...
                    WHERE TRUNC(POR_CREATE_DT) >= trunc(sysdate)-90   
                    ...
                    WHERE TRUNC(POT_CREATE_DT) >= trunc(sysdate)-90 
